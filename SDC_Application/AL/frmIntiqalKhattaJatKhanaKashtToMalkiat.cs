...
{
    public partial class frmIntiqalKhattaJatKhanaKashtToMalkiat : Form
    {
        ...
        public frmIntiqalKhattaJatKhanaKashtToMalkiat()
        {
            InitializeComponent();
        }
        ...
        {
            // visible / Hide Amaldaramad Button for users
                    ...
                    if (GridViewInteqalKhattas.SelectedRows.Count > 0)
                    {
                        ...
                        if (row.Selected)
                        {
                            ...
                            if (IntiqalKhataId != string.Empty)
                            {
                                ...
                                if (this.FardTokenId != "0" && this.FardTokenId != null)
                                    AutoFillCombo.FillCombo("proc_Self_Get_Khatoonies_of_Khata_for_RegistryIntiqal_By_FardTokenId "+UsersManagments._Tehsilid.ToString()+", '" + IntiqalKhataId + "'" + "," + FardTokenId, cmbKhatoniNo, "KhatooniNo", "KhatooniId");
                                else
                                    AutoFillCombo.FillCombo("dbo.Proc_Get_Khatoonis "+UsersManagments._Tehsilid.ToString()+",'" + IntiqalKhataId + "'", cmbKhatoniNo, "KhatooniNo", "KhatooniId");
                               
                                
                                GetKhatoni(txtKhattaRecId.Text);
                                if (this.FardTokenId != "0" && this.FardTokenId != null)
                                    AutoFillCombo.FillCombo("proc_Self_Get_RegistryIntiqal_Khata_Malkan " + UsersManagments._Tehsilid.ToString() + ",'" + IntiqalKhataId + "'" + "," + FardTokenId, cboPersonSeller, "personname", "KhewatGroupFareeqId");
                                else
                                    AutoFillCombo.FillCombo("proc_Get_Intiqal_Khata_Malkan " + UsersManagments._Tehsilid.ToString() + ",'" + IntiqalKhataId + "'", cboPersonSeller, "personname", "KhewatGroupFareeqId");
                            ...
                            //FillgridByBuyerList();
                            btnKhatooniAllMushtryanSave.Visible = false;
                            txtKhatooniDetail.Clear();
                            btnSaveAllMushtryanofSelectedKhatoonies.Text = "کھاتہ " + GridViewInteqalKhattas.CurrentRow.Cells["KhataNo"].Value.ToString() + " کے منتخب شدہ کھتونی کے تمام مشتریان محفوظ کریں ";
                            if (GridBuyersList.Rows.Count > 0)
                            {
                                btnSaveAllMushtryanofSelectedKhatoonies.Enabled = false;
                            }
                            else
                            {
                                btnSaveAllMushtryanofSelectedKhatoonies.Enabled = true;
                            }       
                        }
                        else
                        {
        ...
        private void btnKhatoniSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string intiqalId = this.IntiqalId;
                string KashtkaranDetails = txtKhatooniDetail.Text;
                if (cmbKhatoniNo.SelectedIndex != 0)
                {
                    if (grdKhatoniDetails.Rows.Count > 0)
                    {
                        //if (txtKhatooniDetail.Text.Trim().Length == 0)
                        //{
                        //    MessageBox.Show("کھتونی کی تفصیل درج کریں", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        //    return;
                        //}
                        for (int i = 0; i < this.grdKhatoniDetails.Rows.Count; i++)
                        {
                            if (this.cmbKhatoniNo.SelectedValue.ToString() == this.grdKhatoniDetails.Rows[i].Cells["KhatooniId"].Value.ToString() && txtIntiqalKhatooniRecId.Text=="-1")
                            {
                                duplication = false;
                                break;
                            }
                        ...
                        if (duplication)
                        {
                            ...
                            string LastId = inteq.SaveintiqalKhatoniWithKhashtkaranDetail(IntiqalKhatooniRecId, intiqalId, IntiqalKhataRecId, IntiqalKhataId, KhatoniId,KashtkaranDetails, UsersManagments.UserId.ToString(), UsersManagments.UserName.ToString());
        ...
        public void GetKhatoni(string IntiqalKhataRecId)
        {
            ...
            {
                DataTable dt = new DataTable();
                dt = inteq.GetKhatoniDetailsWithKashtkaran(IntiqalKhataRecId);
        ...
        private void grdKhatoniDetails_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1)
            {
                try
                {
                        ...
                        foreach (DataGridViewRow row in g.Rows)
                        {
                            if (grdKhatoniDetails.SelectedRows.Count > 0)
                            {
                                ...
                                if (row.Selected)
                                {
                                    ...
                                    txtKhatooniDetail.Text = grdKhatoniDetails.CurrentRow.Cells["KhatooniKashtkaranFullDetail_New"].Value.ToString();
                                   ...
                                   // if (Convert.ToDouble(IntiqalKhataRecId) > 0)
                                    ...
                                    btnKhatooniAllMushtryanSave.Text = "کھتونی " + this.grdKhatoniDetails.CurrentRow.Cells["KhatooniNo"].Value.ToString() + " کے تمام مشتریان محفوظ کریں ";
                                    btnKhatooniAllMushtryanSave.Visible = true;
                                    if(GridSellerList1.Rows.Count>0)
                                    {
                                        btnKhatooniAllMushtryanSave.Enabled = false;
                                    }
                                    else
                                    {
                                        btnKhatooniAllMushtryanSave.Enabled = true;
                                    }
                                }
                                else
                                {
                    ...
                    this.IsKhattaMalkan = false;
                    if (this.FardTokenId != "0" && this.FardTokenId != null)
                        AutoFillCombo.FillCombo("proc_Self_Get_RegistryIntiqal_KhanaKashtKhatooni_Malkan "+UsersManagments._Tehsilid.ToString()+", '" + Khatoniid + "'" + "," + FardTokenId, cboPersonSeller, "personname", "MushtriFareeqId");
                    else
                        AutoFillCombo.FillCombo("proc_Get_Intiqal_KhanaKashtKhatooni_Malkan "+UsersManagments._Tehsilid.ToString()+",'" + Khatoniid + "'", cboPersonSeller, "personname", "MushtriFareeqId");
        ...
        {
            AutoFillCombo.FillCombo("Proc_Get_Khatoonis_ByParentKhatooni "+UsersManagments._Tehsilid.ToString()+"," + ParentKhatooniId + "," + this.IntiqalId, cmbMinKhatooniByParentKhatooni, "KhatooniNo", "KhatooniId");
        }
        ...
        public void FillGridIntiqalKhataRecId()
        {
            try
            {
               ...
               // GridSellerList1.Columns["colMutwafiKhataJat"].DisplayIndex = 12;
                ...
                //GridSellerList1.Columns["colMutwafiKhataJat"].Visible = false;
                        ...
                        if (GridSellerList1.SelectedRows.Count > 0)
                        {
                            ...
                            if (row.Selected)
                            {
                                ...
                                if (row.Cells["KhewatGroupFareeqId"].Value.ToString() == ""  ||row.Cells["KhewatGroupFareeqId"].Value.ToString() == null || row.Cells["KhewatGroupFareeqId"].Value.ToString()=="0")
                                {
                                    ...
                                    txtKulHissayWOTminhay.Text = row.Cells["Seller_Total_Hissa"].Value.ToString();
                                    this.txtKulHisay.Text = row.Cells["Seller_Total_Hissa"].Value.ToString();
                                    this.txtKullKanal.Text = row.Cells["Seller_Total_Kanal"].Value.ToString();
                                    this.txtKullMarla.Text = row.Cells["Seller_Total_Marla"].Value.ToString();
                                    this.txtKullSarsai.Text = row.Cells["Seller_Total_Sarsai"].Value.ToString();
                                    this.txtKulFeet.Text = row.Cells["Seller_Total_Feet"].Value.ToString();
            ...
            try
            {
                ...
                if (chkMushtharaqa.Checked)
                {
                    ...
                    if (dt != null)
                    {
                       ...
                       // MessageBox.Show(" ریکارڈز محفوظ  ھو چکے ہیں", "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
        ...
        private void btnFamilySelection_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (cboKhewatType.SelectedIndex > 0)
                {
                    ...
                    {
                        SPNI.KhatoniRecid = this.txtIntiqalKhatooniRecId.Text;
                    }
        ...
        private void btnDeleteChange_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                AutoFillCombo.FillCombo("Proc_Get_Khatoonis_ByParentKhatooni "+UsersManagments._Tehsilid.ToString()+"," + txtparentKhatooniId.Text + "," + this.IntiqalId, cmbMinKhatooniByParentKhatooni, "KhatooniNo", "KhatooniId");
        ...
        public void Fillkhatoniforkhasra()
        {
            ...
            on.FillCombo("Proc_Get_Khatoonis "+UsersManagments._Tehsilid.ToString()+"," + RegisterHqDKhataId + "", this.cmbkhatoonisnew, "KhatooniNo", "KhatooniId");
        ...
        private void btnSaveChangeKhatataqseem_Click(object sender, EventArgs e)
        {
            if (this.txtparentKhataId.Text.Trim() != "-1" && txtParentid.Text.Trim() != "-1")
            {
                ...
                {
                    AutoComplete on = new AutoComplete();
                    on.FillCombo("Proc_Get_Moza_Register_KhataJat_ParentKhataID "+UsersManagments._Tehsilid.ToString()+"," + MozaId + "," + txtparentKhataId.Text + "", cmbtaqseemChangeKhata, "KhataNo", "RegisterHqDKhataId");
        ...
        private void btnDeleteChange_For_Khata_Click(object sender, EventArgs e)
        {
            if (this.txtRegHaqKhataID.Text != "")
            {
                try
                {
                    ...
                    AutoComplete on = new AutoComplete();
                    on.FillCombo("Proc_Get_Moza_Register_KhataJat_ParentKhataID "+UsersManagments._Tehsilid.ToString()+"," + MozaId + "," + ParentKhataID + "", cmbtaqseemChangeKhata, "KhataNo", "RegisterHqDKhataId");
                    ClearTaqseemNewTab();
                }
                catch (Exception ex)
                {
        ...
        private void tabControl1_Selected(object sender, TabControlEventArgs e)
        {
            ...
            else if (tabControl1.SelectedIndex == 3)
            {
                ...
                ac.FillCombo("Proc_Get_Intiqal_MinGroupsList "+UsersManagments._Tehsilid.ToString()+"," + this.IntiqalId + "," + this.IntiqalKhataId + ",0", cboMinGroups, "IntiqalMinGroupNo", "IntiqalMinGroupId");
            }
        }
        ...
        private void cboPersonSeller_DropDownClosed(object sender, EventArgs e)
        {
            
        }

        private void gbKhataMainContols_Enter(object sender, EventArgs e)
        {
        ...
        {
            SaveAllMushtryanofKhatooni(IntiqalKhataRecId, KhatoniRecid, Khatoniid);
            proc_Get_Intiqal_Sellers_List_ByKhata(IsKhattaMalkan ? IntiqalKhataRecId : "-1", IsKhattaMalkan ? "-1" : KhatoniRecid);
            ClearAll();
            if (GridSellerList1.Rows.Count > 0)
            {
        ...
        private void SaveAllMushtryanofKhatooni(string KhataRecId, string KhatoniRecId, string Khatoniid)
        {
            try
            {

                string LastId = inteq.SaveAllMushtryanofKhatooni(KhataRecId, KhatoniRecId,Khatoniid, UsersManagments.UserId.ToString());
        ...
        private void txtKhatooniDetail_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar != 22 && e.KeyChar != 24 && e.KeyChar != 3 && e.KeyChar != 1 && e.KeyChar != 13)
            {
                if (e.KeyChar == Convert.ToChar((Keys.Back)))
        ...
        private void btnSaveAllMushtryanofSelectedKhatoonies_Click(object sender, EventArgs e)
        {
            SaveAllMushtryanofAllKhatooni(IntiqalKhataRecId);
            FillgridByBuyerList(IntiqalKhataRecId, "-1");
            CalculateSellerBuyerRaqbaHissa();
            if (GridBuyersList.Rows.Count > 0)
            {
        ...
        private void SaveAllMushtryanofAllKhatooni(string KhataRecId)
        {
            try
            {

                string LastId = inteq.SaveAllMushtryanofAllKhatooni(KhataRecId, UsersManagments.UserId.ToString(), UsersManagments.UserName.ToString());
