...
{
    public partial class frmTokenOtherTehsil : Form
    {
       ...
       public void Fill_ListBoxes()
        {
            ...
            //objauto.FillCombo("select ServiceTypeId, ServiceTypeName_Urdu from SDC_ServiceTypes where ServiceTypeName_Urdu=N'فرد'  and TehsilId=" + SDC_Application.Classess.UsersManagments._Tehsilid.ToString(), cmbServiceId, "ServiceTypeName_Urdu", "ServiceTypeId");
            ...
            objauto.FillCombo("Proc_Entry_DistrictsAll ", cmdDistrict, "DistrictnameUrdu", "DistrictId");
            //if(cmbServiceId.DataSource != null)  if(cmbServiceId.Items.Count>1)cmbServiceId.SelectedIndex = 1 ;
            //if (cmbPurpose.DataSource != null) if (cmbPurpose.Items.Count > 1) cmbPurpose.SelectedIndex = 1;
       }
       public void loadTokenNo()
       {
           if(cmbTehsil.DataSource!=null)
           {
           dt = this.objbusines.filldatatable_from_storedProcedure("Proc_Get_SDC_Token_Detail_Last " + cmbTehsil.SelectedValue.ToString() );
           if (dt != null)
           {
       ...
       private void txtVisitorName_KeyPress_1(object sender, KeyPressEventArgs e)
       {
        ...
        
        {
            ...
            collection.Add(this.cbRelation.Text.Trim().ToString());
            ...
            if (empty==null)//this.cmbMouza.SelectedIndex != 0 && this.txtFatherHusband.Text != string.Empty && this.txtVisitorName.Text != string.Empty && this.txtTempAddress.Text != string.Empty && cmbPurpose.SelectedIndex != 0 && cmbServiceId.SelectedIndex != 0 && txtCNCI.TextLength == 15)
                {
                    if (cbRelation.Text.Trim().Length > 0 && cmbTehsil.SelectedValue.ToString()!="0")
                    {
                        if (Submit())
                        {
                        ...
                        MessageBox.Show("درخواست دہندہ کا دلدیت یا زوجیت (رشتہ) منتخب کریں۔ یا تحصیل کا انتخاب نہیں کیا ہے۔", "انتخاب رشتہ",MessageBoxButtons.OK, MessageBoxIcon.Error );
                }
                else
                {
        ...
        public bool Submit()
        {
            try
            {
                if (MessageBox.Show("کیا آپ محفوظ کرنا چاہتے ہیں:::::", "محفوظ", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    WEB_SP_INSERT_SDC_Tokens_OtherDistrictTehsils(UsersManagments.UserName.ToString(), UsersManagments.UserId.ToString(), TokenVerified, txttokenid, txtToken, status1, TokenDuplicate, cbRelation.Text, cmbTehsil.SelectedValue.ToString());
                    return true;
                }
            }
            ...
            if (chkverified.Enabled == true)
            {
                ...
                try
                {
                        ...
                        == DialogResult.Yes)
                    {
                        ...
                        TokenDuplicate = "False";
                        // txtLoad.Text = "";                          
                        WEB_SP_INSERT_SDC_Tokens(cmbTehsil.SelectedValue.ToString(), UsersManagments.UserName.ToString(), UsersManagments.UserId.ToString(), TokenVerified, txttokenid, txtToken, status2, TokenDuplicate, cbRelation.Text);
        ...
        public void WEB_SP_INSERT_SDC_Tokens(string TehsilId, string username, string UserID, string tokenverified, TextBox tokenid, TextBox tokenno, TextBox status,string TokenDuplicate, string Relation)
        {



                                    dt = objbusines.filldatatable_from_storedProcedure(
                                    "WEB_SP_INSERT_SDC_Tokens '" + tokenid.Text.ToString() + "'," + TehsilId+ "," + UsersManagments._LocationId + ",'"
                                    + datetoken + "','" 
        ...
        public void WEB_SP_INSERT_SDC_Tokens_OtherDistrictTehsils(string username, string UserID, string tokenverified, TextBox tokenid, TextBox tokenno, TextBox status, string TokenDuplicate, string Relation, string TehsilId)
        {
        ...
        private void btnCancel_Click(object sender, EventArgs e)
        {
            ...
            if (this.chkverified.Checked==false && txtStatus.Text=="محفوظ شدہ")
            {
                ...
                {
                    WEB_SP_INSERT_SDC_Tokens(cmbTehsil.SelectedValue.ToString(), UsersManagments.UserName.ToString(), UsersManagments.UserId.ToString(), TokenVerified, txttokenid, txtToken, status3, TokenDuplicate, cbRelation.Text);
        ...
        private void btnPopulate_Click(object sender, EventArgs e)
        {
            if (cmbTehsil.DataSource != null)
            {
                if (cmbTehsil.SelectedValue.ToString() != "0")
                {
                    frmTokenPopulate Populate = new frmTokenPopulate();
                    Populate.otherTehsilId = cmbTehsil.SelectedValue.ToString();
                ...
                MessageBox.Show("تحصیل کا انتخاب کریں");
        }
        ...
        private void cmbServiceId_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cmbServiceId.SelectedIndex > 0)
            {
                ...
                {
                    objauto.FillCombo("Proc_Get_SDC_TokenPurpose_Fard " + cmbTehsil.SelectedValue.ToString(), cmbPurpose, "TokenPurpose_Urdu", "TokenPurposeId");
                }
                else
                {
                    objauto.FillCombo("Proc_Get_SDC_TokenPurpose_Intiqal " + cmbTehsil.SelectedValue.ToString(), cmbPurpose, "TokenPurpose_Urdu", "TokenPurposeId");
                }
        ...
        private void btnSearchByNIC_Click(object sender, EventArgs e)
        {
            ...
            if (dt != null)
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    txttokenid.Text = dr["TokenId"].ToString();
                    //txtDate.Text = dr["TokenDate"].ToString();
                    //this.txttokenid.Text = dr["TokenId"].ToString();
                    //this.txtToken.Text = dr["TokenNo"].ToString();
                   // this.labeltimetoken.Text = dr["TokenTime"].ToString();
        ...
        private void cmbTehsil_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            objauto.FillCombo("Proc_Get_Moza_List " + cmbTehsil.SelectedValue.ToString(), cmbMouza, "MozaNameUrdu", "MozaId");
            objauto.FillCombo("select ServiceTypeId, ServiceTypeName_Urdu from dbo.fn_SDC_ServiceTypes(" + cmbTehsil.SelectedValue.ToString() + ") where ServiceTypeName_Urdu=N'فرد'  and TehsilId=" + cmbTehsil.SelectedValue.ToString(), cmbServiceId, "ServiceTypeName_Urdu", "ServiceTypeId");
            //objauto.FillCombo("Proc_Get_Moza_List " + SDC_Application.Classess.UsersManagments._Tehsilid.ToString(), cmbMouza, "MozaNameUrdu", "MozaId");
            //objauto.FillCombo("Proc_Entry_DistrictsAll ", cmdDistrict, "DistrictnameUrdu", "DistrictId");
            if (cmbServiceId.DataSource != null) if (cmbServiceId.Items.Count > 1) cmbServiceId.SelectedIndex = 1;
            if (cmbPurpose.DataSource != null) if (cmbPurpose.Items.Count > 1) cmbPurpose.SelectedIndex = 1;
        ...
        private void btnGenerateFard_Click(object sender, EventArgs e)
        {
            if (cmbMouza.DataSource != null && cmbTehsil.DataSource != null)
            {
                if (txttokenid.Text.Length > 10 && cmbTehsil.SelectedValue.ToString() != "0" && cmbMouza.SelectedValue.ToString().Length > 3)
                {
                    frmShortFardForOtherDistTehsil shortFard = new frmShortFardForOtherDistTehsil();
                    shortFard.SelectedMozaId = cmbMouza.SelectedValue.ToString();
                    shortFard.SelectedTokenId = txttokenid.Text;
                    shortFard.SelectedTokenNo = label12.Text;
                    shortFard.TehsilId = cmbTehsil.SelectedValue.ToString();
                    shortFard.ShowDialog();
                    //MessageBox.Show("Service Type Id:" + cmbServiceId.SelectedValue.ToString() + ", Purpose Id:" + cmbPurpose.SelectedValue.ToString());
                }
                else
                    MessageBox.Show("ٹوکن کا انتخاب کریں");
            }
            else
                MessageBox.Show("تحصیل اور موضع کا انتخاب کریں");
        }

        private void cmdDistrict_SelectionChangeCommitted(object sender, EventArgs e)
        {
            objauto.FillCombo("Proc_Entry_Tehsils " + cmdDistrict.SelectedValue.ToString(), cmbTehsil, "TehsilNameUrdu", "TehsilId");
        }
        ...
        private void btnCheckRecord_Click(object sender, EventArgs e)
        {
            if (cmbTehsil.DataSource != null)
            {
                if (cmbTehsil.SelectedValue.ToString() != "0" && cmbMouza.SelectedValue.ToString().Length > 3)
                {
                    frmKhattaSearchByPersonForOtherDistTehsil frmseachPerson = new frmKhattaSearchByPersonForOtherDistTehsil();
                    frmseachPerson.TehsilId = cmbTehsil.SelectedValue.ToString();
                    frmseachPerson.MozaID = cmbMouza.SelectedValue.ToString();
                    frmseachPerson.ShowDialog();
                }
            }
        }
