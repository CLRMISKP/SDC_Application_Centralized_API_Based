...
namespace SDC_Application.AL
{
    public partial class frmIntiqalRevenueReports : Form
    {
        ...
        private void frmIntiqalRevenueReports_Load(object sender, EventArgs e)
        {
            ...
            if(this.AmalDaramad)
            {
                btnDetailIntiqalInderaj.Enabled=false;
            }
        }
        ...
        private void btnDetailIntiqalInderaj_Click(object sender, EventArgs e)
        {
            ...
            //if (txtTehsilReport.Text.Trim().Length == 0)// && Intiq.ChecIntiqalEnteredTaxReceived(tokenId) != "1")
            //{

            //    //MessageBox.Show("پہلے تمام درج شدہ ٹیکسز وصول کریں۔", "انتقال رپورٹ", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            //{
                ...
                Intiq.SaveIntiqalMainReports(IntiqalId, PatwarReport, PRDate, GirdawrReport, GRDate, TehsilReport, TRDate, txtTaxDetails.Text.Trim(), Classess.UsersManagments.UserId.ToString());
        ...
        private void btnGirdawarReport_Click(object sender, EventArgs e)
        {
            try
            {
                DataTable dt = new DataTable();
                dt = Intiq.Proc_Get_Intiqal_Girdawar_Report(IntiqalId);
                if (dt.Rows.Count > 0)
                {

                    foreach (DataRow d in dt.Rows)
                    {
                        txtGirdawrReport.Text = d["GirdawarReport"].ToString();
                    }
                }
            }
            catch (Exception ex)
