...
{
    public partial class frmCovid19Token : Form
    {
       ...
       public void Fill_ListBoxes()
        {
            objauto.FillCombo("Proc_Get_ServiceTypes_All " + SDC_Application.Classess.UsersManagments._Tehsilid.ToString() + ",", cmbServiceId, "ServiceTypeName_Urdu", "ServiceTypeId");
            objauto.FillCombo("Proc_Get_SDC_TokenPurpose " + SDC_Application.Classess.UsersManagments._Tehsilid.ToString() + ",", cmbPurpose, "TokenPurpose_Urdu", "TokenPurposeId");
            objauto.FillCombo("Proc_Get_Moza_List  " + SDC_Application.Classess.UsersManagments._Tehsilid.ToString() + "," + SDC_Application.Classess.UsersManagments._Tehsilid.ToString(), cmbMouza, "MozaNameUrdu", "MozaId");
       }

        public void loadBooking()
       {
           DataTable dtTokens = new DataTable();
           dtTokens = search.GetTokenDetailByDateCovid();
           dgvBooking.DataSource = dtTokens;
           dgvBooking.Columns["PurposeTypeId"].Visible = false;
           dgvBooking.Columns["MozaId"].Visible = false;
           dgvBooking.Columns["ServiceTypeId"].Visible = false;
           dgvBooking.Columns["BookingId"].Visible = false;
           dgvBooking.Columns["سہولت"].Visible = false;
           dgvBooking.Columns["TokenNo"].HeaderText = "ٹوکن نمبر";
           //dgvBooking.Columns["TokenNo"].Visible = false;
           //dgvBooking.Columns["TokenNo"].HeaderText = "بکنگ نمبر";
           dgvBooking.Columns["ٹوکن تاریخ"].Visible = false;
       }
       ...
       private void txtVisitorName_KeyPress_1(object sender, KeyPressEventArgs e)
       {
        ...
        public bool Submit()
        {
            try
            {
                if (MessageBox.Show("کیا آپ محفوظ کرنا چاہتے ہیں:::::", "محفوظ", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    WEB_SP_INSERT_SDC_Tokens(UsersManagments.UserName.ToString(), UsersManagments.UserId.ToString(), TokenVerified, txttokenid, txtToken, status1, TokenDuplicate,this.BookingId);
                    return true;
                }
            }
            ...
            if (chkverified.Enabled == true)
            {
                ...
                try
                {
                        ...
                        == DialogResult.Yes)
                    {
                        ...
                        TokenDuplicate = "False";
                        // txtLoad.Text = "";                          
                        WEB_SP_INSERT_SDC_Tokens(UsersManagments.UserName.ToString(), UsersManagments.UserId.ToString(), TokenVerified, txttokenid, txtToken, status2, TokenDuplicate,"-1");
        ...
        public void WEB_SP_INSERT_SDC_Tokens(string username, string UserID, string tokenverified, TextBox tokenid, TextBox tokenno, TextBox status,string TokenDuplicate, string BookingId)
        {



                                    dt = objbusines.filldatatable_from_storedProcedure(
                                    "Covid19_WEB_SP_INSERT_SDC_Tokens '" + tokenid.Text.ToString() + "'," + UsersManagments._Tehsilid + ",'"
                                    + datetoken + "','" 
                                    ...
                                    "','" + UserID + "','"
                                    + username + "','" + UserID + "','" + username + "',"+ BookingId);
        ...
        private void btnCancel_Click(object sender, EventArgs e)
        {
            ...
            if (this.chkverified.Checked==false && txtStatus.Text=="محفوظ شدہ")
            {
                ...
                {
                    WEB_SP_INSERT_SDC_Tokens(UsersManagments.UserName.ToString(), UsersManagments.UserId.ToString(), TokenVerified, txttokenid, txtToken, status3, TokenDuplicate,"-1");
        ...
        {
            //dt = this.objbusines.filldatatable_from_storedProcedure("Covid19_Proc_Get_SDC_BookingDetail_By_Person_CNIC '" + this.txtCNCI.Text.Trim() + "'");
            //if (dt != null)
            //{
            ...
            //        txtVisitorName.Text = dr["نام"].ToString();
            //        txtCNCI.Text = dr["شناختی کارڈ"].ToString();
            //        txtVisitorContactNo.Text = dr["رابطہ نمبر"].ToString();
            //        txtFatherHusband.Text = dr["والد"].ToString();
            //        txtTempAddress.Text = "";
            //        cmbMouza.SelectedValue = dr["MozaId"];
            //        txtParAdress.Text = "";
            //        cmbPurpose.SelectedValue = dr["PurposeTypeId"].ToString();
            ...
            DataTable dtTokens = new DataTable();
            dtTokens = search.GetTokenDetailByPersonCNICCovid(this.txtCNCI.Text.Trim());
            dgvBooking.DataSource = dtTokens;
            dgvBooking.Columns["PurposeTypeId"].Visible = false;
            dgvBooking.Columns["MozaId"].Visible = false;
            dgvBooking.Columns["ServiceTypeId"].Visible = false;
        ...
        private void dgPersonTokenDetails_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    cmbMouza.SelectedValue = g.SelectedRows[0].Cells["Mozaid"].Value.ToString();
                    ...
                    txtVisitorName.Text = g.SelectedRows[0].Cells["نام"].Value.ToString();
                    txtFatherHusband.Text = g.SelectedRows[0].Cells["والد کا نام"].Value.ToString();
                    txtVisitorContactNo.Text = g.SelectedRows[0].Cells["رابطہ نمبر"].Value.ToString();
                   
                    txtCNCI.Text = g.SelectedRows[0].Cells["شناختی کارڈ"].Value.ToString();
        ...
        private void dgvBooking_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (g.SelectedRows.Count > 0)
                {
                    ...
                    {
                        MessageBox.Show(" اس بکنگ پر پہلے سے ٹوکن نمبر  " + g.SelectedRows[0].Cells["TokenNo"].Value.ToString() + " جاری ہو چکا ہے", "اندراج ٹوکن", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    else
                    {
                        clearTextFields();
                        txtVisitorName.Text = g.SelectedRows[0].Cells["نام"].Value.ToString();
                        txtFatherHusband.Text = g.SelectedRows[0].Cells["والد"].Value.ToString();
                        txtVisitorContactNo.Text = g.SelectedRows[0].Cells["رابطہ نمبر"].Value.ToString();
                        txtCNCI.Text = g.SelectedRows[0].Cells["شناختی کارڈ"].Value.ToString();
                        cmbMouza.SelectedValue = Convert.ToInt32(g.SelectedRows[0].Cells["Mozaid"].Value.ToString());
                        cmbServiceId.SelectedValue = Convert.ToInt32(g.SelectedRows[0].Cells["ServiceTypeId"].Value.ToString());
                        cmbPurpose.SelectedValue = Convert.ToInt32(g.SelectedRows[0].Cells["PurposeTypeId"].Value.ToString());
                        this.BookingId = g.SelectedRows[0].Cells["BookingId"].Value.ToString();

                        txtCNCI.Enabled = false;
        ...
        {
            if (dgvBooking.ColumnCount > 0)
            {
                (dgvBooking.DataSource as DataTable).DefaultView.RowFilter = string.Format("نام LIKE '{0}%' OR نام LIKE '% {0}%'", txtSearch.Text);
            }
        }
