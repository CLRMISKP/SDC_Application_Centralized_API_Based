    ...
    public partial class frmNaqalIntiqal : Form
    {
        ...
        private void btnListIntiqals_Click(object sender, EventArgs e)
        {
            if (this.MozaId != "")
            {
                ...
                IntiqalList = this.objbusines.filldatatable_from_storedProcedure("proc_Get_Intiqal_List_For_Naqal " + SDC_Application.Classess.UsersManagments._Tehsilid.ToString() + ",'" + IntiqalNo + "', '" + Sellers1 + "','" + Sellers2 + "','" + Sellers3 + "','" + Buyers1 + "','" + Buyers2 + "','" + Buyers3 + "','" + this.MozaId + "'");
                if (IntiqalList != null)
                {
                    this.GridWitnessDetails.DataSource = IntiqalList;
                    //int ind=0;
                    //foreach(DataRow dr in IntiqalList.Rows)
                    //{
                    //    GridWitnessDetails.Rows.Add();
                    //    GridWitnessDetails.Rows[ind].Cells[0].Value=dr["IntiqalId"];
                    //    GridWitnessDetails.Rows[ind].Cells[1].Value=dr["IntiqalNo"];
                    //    GridWitnessDetails.Rows[ind].Cells[2].Value=dr["IntiqalAndrajDate"];
                    //    GridWitnessDetails.Rows[ind].Cells[3].Value=dr["IntiqalAmaldramadDate"];
                    //    GridWitnessDetails.Rows[ind].Cells[4].Value=dr["AmalDaramadStatus"];
                    //    GridWitnessDetails.Rows[ind].Cells[5].Value=dr["IntiqalPendingRemakrs"];
                    //    GridWitnessDetails.Rows[ind].Cells[6].Value = ResizeImages.ResizeImage(Resource1.pdf2, Resource1.pdf2.Width, Resource1.pdf2.Height, false);//
                    //    //dr["IntiqalDocImage"];
                    //    ind++;
                    //}
                    if (IntiqalList != null)
                    {
                        ...
                        //GridWitnessDetails.Columns["IntiqalDocImage"].HeaderText = "نقل انتقال";

                        //GridWitnessDetails.Columns["PersonPic"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
                        //GridWitnessDetails.Columns["PersonFingerPrint"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
                        //GridWitnessDetails.Columns["CompleteName"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
                    }
        ...
        private void GridWitnessDetails_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 0)
            {
                ...
                DataTable IntiqaDocImage = this.objbusines.filldatatable_from_storedProcedure("proc_Get_Intiqal_DocImage_For_Naqal " + SDC_Application.Classess.UsersManagments._Tehsilid.ToString() + ",'" + IntId + "'");
