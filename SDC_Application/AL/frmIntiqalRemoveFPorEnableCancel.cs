...
{
    public partial class frmIntiqalRemoveFPorEnableCancel : Form
    {
        byte[] DeSerializee; 
        ...
        public frmIntiqalRemoveFPorEnableCancel()
        {
            InitializeComponent();
        }
        ...
        {
            if (this.txtIntiqalRevertComments.Text.Trim().Length > 10)
            {
                string lastId = Intiqal.ReturnAttestedOrCancelledIntiqal(this.intiqalId.ToString(), cboROs.SelectedValue.ToString(), txtIntiqalRevertComments.Text.Trim());
                if (lastId == "0")
                {
                    MessageBox.Show("کسی واجہ سے انتقال واپس نہیں ہو سکا۔ دوبارا کوشش کریں یا ایس ڈی سی منیجر سے رابطہ کریں۔");
                    return;
                }
            ...
            {
                MessageBox.Show("تفصیل واپسی انتقال میں کم از کم ۱۰ حروف لکھنا ضروری ہے۔");
            }
        }

        #region Auto Language Conversion for Intiqal Revert Comments
  
        private void txtIntiqalRevertComments_KeyPress(object sender, KeyPressEventArgs e)
        {
