        ...
        private void calculateKhassrasRaqba()
        {
            ...
            foreach (DataGridViewRow row in gridViewKhassraAreaDetails.Rows)
            {
                try
                {
                    ...
                    sarsai += row.Cells[8].Value != null ? float.Parse(row.Cells[8].Value.ToString()) : 0;
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message);
            ...
            if (sarsai >= 9)
            {
                marla +=Convert.ToInt32( Math.Round( (sarsai - (sarsai % 9)) / 9, 0));
                sarsai = sarsai % 9;
            }
            if (marla >= 20)
            {
        ...
        private void btnSaveFardBadar_Click(object sender, EventArgs e)
        {
               try 
	                {	        
                        ...
                        string LastId = misalBL.SaveFardBadarMain(fbId, Convert.ToInt32(cmbMouza.SelectedValue.ToString()), txtFardBadarDocNO.Text.Trim(), dtpDateGardawari.Value.ToShortDateString(), dtpDateTehsilDar.Value.ToShortDateString(), txtFardBadarTafseel.Text, UsersManagments.UserId, UsersManagments.UserName);
                        txtFbId.Text = LastId;
