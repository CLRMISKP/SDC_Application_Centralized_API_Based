...
namespace SDC_Application.AL
{
    public partial class frmIntiqalKhattaJatKhatooni : Form
    {
            ...
            if (mingrp == 0)
            {
                ...
                else
                {
                    if (isAttested)
                    {
                        ...
                        this.gbKhatooniMainContols.Enabled = true;
            ...
            else if (mingrp == 1)
            {
                ...
                else
                {
                    if (isAttested)
                    {
                        ...
                        btnAmalDaramadTaqseem.Enabled = true;
                        this.gbKhataMainContols.Enabled = true;
                        this.gbKhatooniMainContols.Enabled = false;
                    ...
                    if (GridViewInteqalKhattas.SelectedRows.Count > 0)
                    {
                        if (row.Selected)
                        {
                            ...
                            if (IntiqalKhataId != string.Empty)
                            {
                                ...
                                if (!MalkiatKashkat)
                                {
                                    AutoFillCombo.FillCombo("dbo.Proc_Get_Khatoonis '" + IntiqalKhataId + "'", cmbKhatoniNo, "KhatooniNo", "KhatooniId");
        ...
        private void grdKhatoniDetails_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1)
            {
                try
                {
                    ...
                    if (!MalkiatKashkat)
                    {
                        AutoFillCombo.FillCombo("proc_Get_Intiqal_KhanaKashtKhatooni_Malkan '" + Khatoniid + "'", cboPersonSeller, "personname", "MushtriFareeqId");
                    }
        ...
        {
            AutoFillCombo.FillCombo("Proc_Get_Khatoonis_ByParentKhatooni " + ParentKhatooniId + "," + this.IntiqalId, cmbMinKhatooniByParentKhatooni, "KhatooniNo", "KhatooniId");
        }
                    ...
                    if (MessageBox.Show("کیا آپ محفوظ کرنا چاہتے ہیں:::::", "محفوظ", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        if (GridSellerList1.Rows.Count > 0)
                        {
                            foreach (DataGridViewRow row in GridSellerList1.Rows)
                            {
                                ...
                                if (this.cboPersonSeller.SelectedValue.ToString() == duplicationCheck)
                                {
                                    isExists = true;
        ...
        private void SaveIntiqalBuyerRecord()
        {
            try
            {
                ...
                string s = inteq.SaveintiqalBuyers(IntiqalBuyerRecId, IntiqalKhataRecId, KhatoniRecid, IntiqalBuyerPersonId, buyerhissay.ToString(), buyerkanal, buyermarla, buyersarsai.ToString(), buyerfeet.ToString(), khewattypeid, BuyerHisaBata).FirstOrDefault().ToString();
        ...
        private void btnSaveMinFareeq_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    inteq.UpdateIntiqalMinFareeqHissa(IntiqalMinFareeqId, IntiqalMinHissa, MinKanal.ToString(), MinMarla.ToString(), MinSarsai, MinSft, this.MozaId.ToString());
        ...
        private void btnDeleteKhatoni_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.GetKhatoni(this.txtKhattaRecId.Text);
            }
            catch (Exception ex)
            {
        ...
        {
            if (dtKhatooniesByParentKhatooni != null?dtKhatooniesByParentKhatooni.Rows.Count > 0:false)
            {
        ...
        private void btnDeleteChange_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                AutoFillCombo.FillCombo("Proc_Get_Khatoonis_ByParentKhatooni " + txtparentKhatooniId.Text + "," + this.IntiqalId, cmbMinKhatooniByParentKhatooni, "KhatooniNo", "KhatooniId");
