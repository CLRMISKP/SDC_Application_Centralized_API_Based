...
namespace EOSDigital.API
{
    ...
    public class CanonAPI : IDisposable
    {
        ...
        {
            ThreadPool.QueueUserWorkItem((state) => {if(CameraAdded!=null)CameraAdded.Invoke(this);});
            return ErrorCode.OK;
        }
            ...
            lock (InitLock)
            {
                if (!IsDisposed)
                {
                    ...
                    if (RefCount == 1)
                    {
                        ...
                        //Terminate the SDK
                        if (MainThread!=null && MainThread.IsRunning == true) err = MainThread.Invoke(() => { return CanonSDK.EdsTerminateSDK(); });
                        //Close the main thread
                        if (MainThread!=null && MainThread.IsRunning == true) MainThread.Shutdown();
