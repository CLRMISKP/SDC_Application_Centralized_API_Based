    ...
    public class Camera : IDisposable
    {
        ...
        private ErrorCode Camera_SDKObjectEvent(ObjectEventID inEvent, IntPtr inRef, IntPtr inContext)
        {
            ThreadPool.QueueUserWorkItem((state) =>
            {
                ...
                if(ObjectChanged!=null)ObjectChanged.Invoke(this, inEvent, inRef);
        ...
        private ErrorCode Camera_SDKProgressCallbackEvent(int inPercent, IntPtr inContext, ref bool outCancel)
        {
            ...
            {
                try {
                    if(ProgressChanged!=null)ProgressChanged.Invoke(this, inPercent); 
                }
                catch (Exception ex) { if (!IsDisposed && !ErrorHandler.ReportError(this, ex)) throw; }
            });
            return ErrorCode.OK;
        ...
        private ErrorCode Camera_SDKPropertyEvent(PropertyEventID inEvent, PropertyID inPropertyID, int inParameter, IntPtr inContext)
        {
            ThreadPool.QueueUserWorkItem((state) =>
            {
                ...
                if(PropertyChanged!=null)PropertyChanged.Invoke(this, inEvent, inPropertyID, inParameter);
            });
            return ErrorCode.OK;
        }
            ...
            ThreadPool.QueueUserWorkItem((state) =>
            {
                try
                {
                    if (inEvent == StateEventID.Shutdown)
                    {
                        SessionOpen = false;
                        Dispose(true);
                        if(CameraHasShutdown!=null)CameraHasShutdown.Invoke(this);
                    }
                }
                catch (Exception ex) { if (!IsDisposed && !ErrorHandler.ReportError(this, ex)) throw; }

                if(StateChanged!=null)StateChanged.Invoke(this, inEvent, inParameter);
            });
            return ErrorCode.OK;
        }
        ...
        private void DownloadEvf()
        {
            ...
            try
            {
                ...
                using (var stream = new SDKStream(0))
                {
                    ...
                    while (KeepLVAlive)
                    {
                        ...
                        //Update live view
                        if(LiveViewUpdated!=null)LiveViewUpdated.Invoke(this, stream);
                    }
                }
            }
            ...
            {
                IsLiveViewOn = false;
                ThreadPool.QueueUserWorkItem((state) => {if(LiveViewStopped!=null)LiveViewStopped.Invoke(this);});
            }
        }
